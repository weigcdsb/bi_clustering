...
for gg in tqdm(range(1,n_iter)): # n_iter
    ...
    for _ in range(fix_rep):
        ...
        delt_b, C_b = update_deltC(delt_tmp, C_tmp, y,np.ones((N,))*r_all, Z_tmp.ravel(), muX_fit[gg], prior)
    
        for ii in range(N):
            eta_ori = delt_tmp[ii] + np.hstack((1, C_tmp[ii,:])) @ muX_fit[gg][Z_fit[gg-1,ii]]
            ...
            try:
                ...
                if np.log(np.random.uniform()) < lhr:
                    ...
                    delt_fit[gg,ii] = copy.deepcopy(delt_tmp[ii])
                    C_fit[gg,ii,:] = copy.deepcopy(C_tmp[ii,:])
        ...
        muX_fit[gg], C_fit[gg,:,:], delt_fit[gg,:], dynamics_fit[gg] = constraint(muX_fit[gg], C_fit[gg,:,:],
                                                                                  ...
                                                                                  dynamics_fit[gg], Z_fit[gg-1,:],muX_tmp)
    ...
    muX_tmp, C_tmp, dynamics_tmp, M1_tmp = contraint_ortho(muX_fit[gg], C_fit[gg,:,:], dynamics_fit[gg],
                                                           Z_fit[gg-1,:])
    ...
    for _ in range(5):
        for zz in Z_more:
            ...
            delt_b, C_b = update_deltC(delt_fit[gg,obsIdx].reshape((-1, 1)), C_fit[gg,obsIdx,:], y_tmp,
                                r_tmp, Z_fit[gg,obsIdx].ravel(), muX_fit[gg], prior)
...
## write...
with open('new_poi_rep_20_55_c1_t_fit.pkl', 'wb') as f: pickle.dump(t_fit, f)
with open('new_poi_rep_20_55_c1_Z_fit.pkl', 'wb') as f: pickle.dump(Z_fit, f)
with open('new_poi_rep_20_55_c1_states_fit.pkl', 'wb') as f: pickle.dump(states_fit, f)
with open('new_poi_rep_20_55_c1_muX_fit.pkl', 'wb') as f: pickle.dump(muX_fit, f)
